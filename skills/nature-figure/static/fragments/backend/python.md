...
def save_pub_py(fig, filename, dpi=600, alignment_qa=False):
    if alignment_qa:
...
- `scripts/audit_panel_alignment.py` — opt-in render-time Matplotlib axes alignment gate for strict QA or submission-ready figures; call it after the final layout draw and before export.
- `scripts/audit_figure_collisions.py` — PyMuPDF geometry audit for strict QA or submission-ready delivery; fix text-text, text-stroke and clipping FAIL findings before delivery.
